...
static int respanso_x11_error_handler(Display *display, XErrorEvent *error) {
    ...
    // X protocol errors are recoverable here. Keep diagnostics but do not
    // terminate the worker on a non-fatal protocol error.
    return 0;
}
...
static int respanso_x11_io_error_handler(Display *display) {
    ...
    // Xlib treats I/O errors as fatal. This callback preserves the reason in
    // x11-native.log before Xlib terminates the process.
    return 0;
}
...
extern "C" int32_t detect_prepare_x11_runtime(const char *log_path) {
    ...
    // This must happen before worker UI/detector/injector/clipboard Xlib use.
