...
// The Astra X11 detector polls the server keyboard state with XQueryKeymap.
// Unlike XRecord/raw device backends, that API cannot distinguish physical
// key transitions from XTest events generated by our own injector. Keep a
// process-wide, nestable suppression counter so the worker can temporarily
// ignore those synthetic transitions while an expansion is being injected.
static SYNTHETIC_INPUT_SUPPRESSION_DEPTH: AtomicUsize = AtomicUsize::new(0);

pub fn begin_synthetic_input_suppression() {
    SYNTHETIC_INPUT_SUPPRESSION_DEPTH.fetch_add(1, Ordering::SeqCst);
}

pub fn end_synthetic_input_suppression() {
    let _ = SYNTHETIC_INPUT_SUPPRESSION_DEPTH.fetch_update(
        Ordering::SeqCst,
        Ordering::SeqCst,
...
pub fn is_synthetic_input_suppressed() -> bool {
    SYNTHETIC_INPUT_SUPPRESSION_DEPTH.load(Ordering::SeqCst) > 0
}
