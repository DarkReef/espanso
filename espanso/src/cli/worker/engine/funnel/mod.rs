                ...
                if source.initialize().is_err() {
                    ...
                    source
                        .eventloop(Box::new(move |event| {
                            // The Astra X11 source uses XQueryKeymap polling and therefore sees
                            // XTest events generated by rEspanso's own injector. Ignore those
                            // keyboard/hotkey transitions while an injection guard is active;
                            // otherwise synthetic Backspace events can be mistaken for the
                            // user's Undo request and recursively corrupt the expansion.
