                ...
                if source.initialize().is_err() {
                    ...
                    source
                        .eventloop(Box::new(move |event| {
                            // Keep physical/XTest modifier state synchronized even while the
                            // Astra X11 detector is suppressing synthetic text events. The
                            // matcher consults this state to reject Alt/Meta modified input; if
                            // a modifier release is swallowed here, the matcher can otherwise
                            // remain permanently disabled until that modifier is pressed again.
                            ...
                            // Update the key state (if needed) before filtering too, so a guarded
                            // injection cannot leave the source-side key snapshot stale.
                            ...
                            // XQueryKeymap also sees rEspanso's own XTest transitions. Suppress
                            // those from the engine/matcher while an injection guard is active,
                            // but only after state bookkeeping above has been reconciled.
                            if espanso_detect::is_synthetic_input_suppressed()
                                && matches!(
                                    &event,
                                    InputEvent::Keyboard(_) | InputEvent::HotKey(_)
                                )
                            {
                                return;
                            }
