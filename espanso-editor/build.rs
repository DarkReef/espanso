...
fn main() {
    ...
    let resource_file = output_dir.join("respanso-match-studio-icon.rc");
    let icon_resource_path = icon_path.to_string_lossy().replace('\\', "/");

    println!("cargo:rerun-if-changed={}", icon_path.display());
    fs::write(&resource_file, format!("1 ICON \"{icon_resource_path}\"\n"))
        .expect("failed to create the rEspanso Match Studio icon resource file");

    let mut resource = winres::WindowsResource::new();
    resource.set_resource_file(
        resource_file
            .to_str()
            .expect("rEspanso Match Studio icon resource path is not valid UTF-8"),
    );
    resource
        .compile()
